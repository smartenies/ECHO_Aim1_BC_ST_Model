...
#####Note: The analysis was updated on September 27, 2018 to include new "friends and family" sites for Campaign 3
We wanted to compare the attributes of Campaign 1 residential locations to those
of the proposed "friends and family" sites for the fall campaign to make sure
we're getting a decent representation of 
...
#' Read in shapefile for Campaign 1 residential and Campaign 3 proposed sites
c1_res <- st_read("./HS_Data/Campaign_1_Residential_Geocoded.shp") %>% 
  st_transform(crs = ll_wgs84)

c3_fnf <- st_read("./HS_Data/Campaign_3_Proposed_Geocoded.shp") %>% 
  st_transform(crs = ll_wgs84)
...
ggmap(base_map) +
  ggtitle("Monitor locations (study participants jittered)") +
  geom_sf(data=st_sf(bound_ll), 
          inherit.aes = F, color="black", fill=NA, size=1) +
  geom_sf(data=mon_ll, 
  ...
  geom_sf(data=st_jitter(c1_res, factor=0.02), 
          inherit.aes = F, aes(color="c1", fill="c1"), pch=20, size=3) +
  geom_sf(data=c3_fnf, 
          inherit.aes = F, aes(color="fnf", fill="fnf"), pch=20, size=3) +
  scale_color_manual(name="Monitor Type",
                     ...
                     labels = c("mon" = "Community", "hs" = "Enrolled", "hs2" = "To Contact",
                                "c1" = "Campaign 1 Res", "fnf" = "Campaign 3 FnF")) +
...
c1_att <- st_transform(c1_res, crs = st_crs(grid_att)) %>% 
  st_join(., grid_att) %>%
  st_transform(crs=ll_wgs84)

c3_att <- st_transform(c3_fnf, crs = st_crs(grid_att)) %>% 
  st_join(., grid_att) %>%
  st_transform(crs=ll_wgs84)
...
kable(c1_att[,-c(1:61)], digits=2,
      caption="Campaign 1 participant cell attributes", row.names = F)
kable(c3_att[,-c(1:61)], digits=2,
      caption="Campaign 3 proposed cell attributes", row.names = F)
```
...
<!-- spplot(grid_att, zcol="tree_cover", -->
...
### Comparing spatial attributes for Campaign 1 participants and proposed new FnF sites

```{r}
c1_df <- as.data.frame(c1_att[,-c(1:59)]) %>% 
  mutate(id = as.character(id))
c3_df <- as.data.frame(c3_att[,-c(1:59)]) %>% 
  mutate(id = as.character(id))

#' subset c3_df2 to just the sites we want to test
sites <- c(1, 2, 3, 4, 7, 8, 13, 6)
c3_df2 <- filter(c3_df, id %in% paste("new_site", sites, sep="_"))
```
...
library(tidyverse)
c1_change <- c1_df
colnames(c1_change) <- gsub("_", ".", colnames(c1_change))

c1_sum <- c1_change %>%
  select(colnames(c1_change[c(4:5,7:10,12:22)])) %>%
  filter(!is.na(tree.cover)) %>% 
  summarise_all(funs(max, median, mean, min)) %>%
  gather(variable, value) %>%
  separate(variable, c("var", "stat"), sep = "\\_") %>%
  spread(var, value)

kable(c1_sum, digits=2)
...
```{r results="asis"}
c3_change <- c3_df2
colnames(c3_change) <- gsub("_", ".", colnames(c3_change))

c3_sum <- c3_change %>%
  select(colnames(c3_change[c(4:5,7:10,12:22)])) %>%
  filter(!is.na(tree.cover)) %>% 
  summarise_all(funs(max, median, mean, min)) %>%
  gather(variable, value) %>%
  separate(variable, c("var", "stat"), sep = "\\_") %>%
...
```{r}
tree_cov <- ggplot() +
  geom_density(data=c1_df, aes(x=c1_df$tree_cover, fill="c1")) + 
  geom_density(data=c3_df2, aes(x=c3_df2$tree_cover, fill="c3"),
               alpha=0.5) + 
  scale_fill_manual(name="Distribution",
                    values = c("c1" = "red", "c3" = "blue"),
                    labels = c("C1 Res cells", "C3 FnF cells")) +
  xlab("Percent Tree Cover") +
  simple_theme
plot(tree_cov)

ks.test(c1_df$tree_cover, c3_df2$tree_cover)
```

#### Percent impervious surfaces
```{r}
imp_surf <- ggplot() +
  geom_density(data=c1_df, aes(c1_df$impervious, fill="c1")) + 
  geom_density(data=c3_df2, aes(c3_df2$impervious, fill="c3"), 
               alpha=0.5) + 
  scale_fill_manual(name="Distribution",
                    values = c("c1" = "red", "c3" = "blue"),
                    labels = c("C1 Res cells", "C3 FnF cells")) +
...
ks.test(c1_df$impervious, c3_df2$impervious)
```

#### Highways
```{r}
high_den <- ggplot() +
  geom_density(data=c1_df, aes(c1_df$road_km_wt, fill="c1")) + 
  geom_density(data=c3_df2, aes(c3_df2$road_km_wt, fill="c3"), 
               alpha=0.5) + 
  scale_fill_manual(name="Distribution",
                    values = c("c1" = "red", "c3" = "blue"),
                    labels = c("C1 Res cells", "C3 FnF cells")) +
...
ks.test(c1_df$road_km_wt, c3_df2$road_km_wt)
```

#### AADT
```{r}
aadt <- ggplot() +
  geom_density(data=c1_df, aes(c1_df$aadt, fill="c1")) + 
  geom_density(data=c3_df2, aes(c3_df2$aadt, fill="c3"),
               alpha=0.5) + 
  scale_fill_manual(name="Distribution",
                    values = c("c1" = "red", "c3" = "blue"),
                    labels = c("C1 Res cells", "C3 FnF cells")) +
...
ks.test(c1_df$aadt, c3_df2$aadt)
```

#### Populaton density
```{r}
pop_den <- ggplot() +
  geom_density(data=c1_df, aes(c1_df$pop_den, fill="c1")) + 
  geom_density(data=c3_df2, aes(c3_df2$pop_den, fill="c3"),
               alpha=0.5) + 
  scale_fill_manual(name="Distribution",
                    values = c("c1" = "red", "c3" = "blue"),
                    labels = c("C1 Res cells", "C3 FnF cells")) +
...
ks.test(c1_df$pop_den, c3_df2$pop_den)
```

#### Housing density
```{r}
housing_den <- ggplot() +
  geom_density(data=c1_df, aes(c1_df$housing_den, fill="c1")) + 
  geom_density(data=c3_df2, aes(c3_df2$housing_den, fill="c3"),
               alpha=0.5) + 
  scale_fill_manual(name="Distribution",
                    values = c("c1" = "red", "c3" = "blue"),
                    labels = c("C1 Res cells", "C3 FnF cells")) +
...
ks.test(c1_df$housing_den, c3_df2$housing_den)

#' SFH density
single_den <- ggplot() +
  geom_density(data=c1_df, aes(c1_df$single_den, fill="c1")) + 
  geom_density(data=c3_df2, aes(c3_df2$single_den, fill="c3"),
               alpha=0.5) + 
  scale_fill_manual(name="Distribution",
                    values = c("c1" = "red", "c3" = "blue"),
                    labels = c("C1 Res cells", "C3 FnF cells")) +
...
ks.test(c1_df$single_den, c3_df2$single_den)

#' MFH density
multi_den <- ggplot() +
  geom_density(data=c1_df, aes(c1_df$multi_den, fill="c1")) + 
  geom_density(data=c3_df2, aes(c3_df2$multi_den, fill="c3"),
               alpha=0.5) + 
  scale_fill_manual(name="Distribution",
                    values = c("c1" = "red", "c3" = "blue"),
                    labels = c("C1 Res cells", "C3 FnF cells")) +
...
ks.test(c1_df$multi_den, c3_df2$multi_den)

#' Mobile home denisty
mobile_den <- ggplot() +
  geom_density(data=c1_df, aes(c1_df$mobile_den, fill="c1")) + 
  geom_density(data=c3_df2, aes(c3_df2$mobile_den, fill="c3"),
               alpha=0.5) + 
  scale_fill_manual(name="Distribution",
                    values = c("c1" = "red", "c3" = "blue"),
                    labels = c("C1 Res cells", "C3 FnF cells")) +
...
ks.test(c1_df$mobile_den, c3_df2$mobile_den)
```
