...
#' 
#' This script focuses on autumn (September, October, November)
#' =============================================================================
...
bc_lur_data <- lur_data %>% 
  dplyr::select(lon, lat, bc_ug_m3, elevation_50:aadt_2500,
...
#' 99
ncol(dplyr::select(bc_lur_data, -c(lon, lat, bc_ug_m3)))

#' Looking at the continuous predictors now
names(bc_lur_data)
bc_continuous <- dplyr::select(bc_lur_data, elevation_50:nearest_3_neighbors_temp)
...
drop_vars2 <- c(drop_vars2, "tree_cover_50", "aadt_100",
                ...
                "impervious_50", "impervious_100",
                "impervious_250", "impervious_1000",
                "nearest_neighbor_pm", "nearest_neighbor_bc",
                "nearest_neighbor_temp", "nearest_neighbor_smoke")
drop_vars2
...
cor(bc_lur_data4$impervious_500, log(bc_lur_data4$bc_ug_m3))
cor(bc_lur_data4$impervious_2500, log(bc_lur_data4$bc_ug_m3))
cor(bc_lur_data4$dist_m_cafos, log(bc_lur_data4$bc_ug_m3))
cor(bc_lur_data4$dist_m_landfill, log(bc_lur_data4$bc_ug_m3))
cor(bc_lur_data4$dist_m_major_rd, log(bc_lur_data4$bc_ug_m3))
...
cor(bc_lur_data4$aadt_50, log(bc_lur_data4$bc_ug_m3))
cor(bc_lur_data4$aadt_500, log(bc_lur_data4$bc_ug_m3))
cor(bc_lur_data4$aadt_1000, log(bc_lur_data4$bc_ug_m3))
...
cor(bc_lur_data4$high_inten_dev_2500, log(bc_lur_data4$bc_ug_m3))

plot(bc_lur_data4$tree_cover_100, log(bc_lur_data4$bc_ug_m3))
plot(bc_lur_data4$impervious_500, log(bc_lur_data4$bc_ug_m3))
plot(bc_lur_data4$impervious_2500, log(bc_lur_data4$bc_ug_m3))
plot(bc_lur_data4$dist_m_cafos, log(bc_lur_data4$bc_ug_m3))
plot(bc_lur_data4$dist_m_landfill, log(bc_lur_data4$bc_ug_m3))
plot(bc_lur_data4$dist_m_major_rd, log(bc_lur_data4$bc_ug_m3))
...
plot(bc_lur_data4$aadt_50, log(bc_lur_data4$bc_ug_m3))
plot(bc_lur_data4$aadt_500, log(bc_lur_data4$bc_ug_m3))
plot(bc_lur_data4$aadt_1000, log(bc_lur_data4$bc_ug_m3))
...
plot(bc_lur_data4$high_inten_dev_2500, log(bc_lur_data4$bc_ug_m3))
...
#' Force BC into the model
log_bc_aic_list <- c(log_bc_aic_list, "nearest_3_neighbors_bc")
...
log_bc_lasso_list # 19 predictors
log_bc_aic_list  # 13 predictors
...
#' 10-fold cross validation: AIC-selected predictors plus BC
#' R2 = 0.85; RMSE = 0.03
...
ggplot(bc_lur_data4) +
  ...
  simple_theme
ggsave(filename = here::here("Figs/Model_Dev", "Log_BC_Meas_vs_Pred_Autumn.jpeg"), 
...
aut_log_bc_aic_cv <- log_bc_aic_cv
aut_log_bc_lasso_cv <- log_bc_lasso_cv

save(aut_log_bc_aic_cv, aut_log_bc_lasso_cv, 
     file = here::here("Results", "BC_LUR_Models_Autumn.rdata"))
...
load(here::here("Results", "BC_LUR_Models_Autumn.rdata"))

grid_data <- read_csv(here::here("Data", "Combined_Grid_Data_AEA.csv")) %>% 
  st_as_sf(wkt = "WKT", crs = albers)

bc_cols <- names(aut_log_bc_aic_cv$finalModel$coefficients)[-1]
bc_cols 
...
grid_data2$pred_log_bc <- predict(aut_log_bc_aic_cv, newdata = grid_data2)
...
#' September 2018
df_plot1 <- filter(grid_data2, month == 9 & year == 2018)
...
plot_name1 <- paste0("BC_Sep_2018_Autumn.jpeg")
ggsave(here::here("Figs", plot_name1),
       device = "jpeg", height= 5, width = 6, dpi = 500, units = "in")

#' October, 2018
df_plot2 <- filter(grid_data2, month == 10 & year == 2018)
...
plot_name2 <- paste0("BC_Oct_2018_Autumn.jpeg")
ggsave(here::here("Figs", plot_name2),
       device = "jpeg", height= 5, width = 6, dpi = 500, units = "in")

#' November, 2018
df_plot3 <- filter(grid_data2, month == 11 & year == 2018)
...
plot_name3 <- paste0("BC_Nov_2018_Autumn.jpeg")
