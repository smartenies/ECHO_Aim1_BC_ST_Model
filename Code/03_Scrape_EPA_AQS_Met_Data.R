...
#' 
#' Description: This script downloads the summary files from the EPA website 
#' ----------------------------------------------------------------------------
...
if(!dir.exists(here::here("secondary_data"))) dir.create(here::here("secondary_data"))
if(!dir.exists(here::here("secondary_data", "epa_aqs_data"))) dir.create(here::here("secondary_data", "epa_aqs_data"))

years <- c(2010:2021)
met_vars <- c("WIND", "PRESS", "TEMP", "RH_DP")

aqs_path <- here::here("secondary_data", "epa_aqs_data")
...
for (i in 1:length(years)) {
  ...
  #' Daily CO: 42101
  aqs_url6 <- paste0("https://aqs.epa.gov/aqsweb/airdata/daily_42101_", 
                     years[i], ".zip")
  
  download.file(aqs_url6, destfile = here::here("data/temp", "temp.zip"))
  unzip(here::here("data/temp", "temp.zip"), exdir = aqs_path)
}
