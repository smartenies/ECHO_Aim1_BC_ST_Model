...
#' names(grid_sp)
#' 
#' grid_sp_cov <- select(grid_sp, grid_id, lon, lat, x, y, all_of(covars)) %>%
#'   distinct() #%>%
#' # mutate_at(.vars = vars(covars),
#' #           scale)
#' 
...
#' grid_end <- as.Date("07/31/2019", format = "%m/%d/%Y")
...
#' 
#' #' Observations
#' bc_obs_df <- as.data.frame(bc_obs2)
#' bc_obs_df$week <- rownames(bc_obs2)
#' 
#' grid_obs_df <- as.data.frame(grid_obs)
#' grid_obs_df$week <- rownames(grid_obs)
#' 
#' obs <- left_join(bc_obs_df, grid_obs_df, by = "week")
#' # obs <- filter(obs, as.Date(week) <= as.Date("2019-03-11"))
#' rownames(obs) <- obs$week
#' obs$week <- NULL
#' obs <- as.matrix(obs)
#' 
#' head(rownames(obs))
...
#' 
#' head(rownames(st_no2))
#' tail(rownames(st_no2))
#' head(colnames(st_no2))
#' tail(colnames(st_no2))
#' 
#' #' smoke
...
#' 
#' head(rownames(st_smk))
#' tail(rownames(st_smk))
#' head(colnames(st_smk))
#' tail(colnames(st_smk))
#' 
#' #' -----------------------------------------------------------------------------
...
#' 
#' head(new_trend)
#' plot(new_trend$date, new_trend$V1, col = 1, pch = 16, cex = 0.5,
#'      xlab = "Date", ylab = "Pollutants (scaled)")
#' points(new_trend$date, new_trend$V2, col = 2, pch = 16, cex = 0.5)
#' lines(new_trend$date, new_trend$V1, col = 1)
#' lines(new_trend$date, new_trend$V2, col = 2)
#' 
#' # Add the smooth temporal basis functions to the ST data object
#' full.data$trend <- new_trend
#' 
#' head(full.data$trend)
#' plot(full.data$trend$date, full.data$trend$V1, col = 1, pch = 16, cex = 0.5,
#'      xlab = "Date", ylab = "Pollutants (scaled)")
#' points(full.data$trend$date, full.data$trend$V2, col = 2, pch = 16, cex = 0.5)
#' lines(full.data$trend$date, full.data$trend$V1, col = 1)
#' lines(full.data$trend$date, full.data$trend$V2, col = 2)
#' 
...
#' par(mar=c(3.3,3.3,1.5,1), mgp=c(2,1,0))
#' layout(matrix(c(1,1,2,2,3,4), 3, 2, byrow=TRUE))
#' plot(full.data, "obs", ID="d_20", xlab="", ylab="BC (log ug/m3)",
#'      main="Temporal trend d_20")
#' plot(full.data, "res", ID="d_20", xlab="", ylab="BC (log ug/m3)")
#' plot(full.data, "acf", ID="d_20")
#' plot(full.data, "pacf", ID="d_20")
#' 
#' par(mar=c(3.3,3.3,1.5,1), mgp=c(2,1,0))
#' layout(matrix(c(1,1,2,2,3,4), 3, 2, byrow=TRUE))
...
#' #' Subset to just 2018:
#' #'
#' #' Because the data set is so large, I'm going to subset the data to just 2018
#' #' to keep the computer from crashing when predicting
#' #' -----------------------------------------------------------------------------
#' 
#' dates <- as.character(seq.Date(as.Date("2018-01-01"), as.Date("2018-12-31"), by = "day"))
#' 
#' obs_sub <- obs[which(rownames(obs) %in% dates),]
#' st_no2_sub <- obs[which(rownames(st_no2) %in% dates),]
#' st_smk_sub <- obs[which(rownames(st_smk) %in% dates),]
#' 
#' sub.data <- createSTdata(obs = obs_sub,
#'                          covars = sp_cov,
#'                          SpatioTemporal = list(bc_st_no2 = st_no2_sub,
#'                                                bc_st_smk = st_smk_sub))
#' sub.data
#' D.sub <- createDataMatrix(sub.data)
#' 
#' sub_trend <- denver.data.B$trend %>%
#'   filter(date %in% as.Date(rownames(obs_sub)))
#' 
#' sub.data$trend <- sub_trend
#' sub.data
#' 
#' names(sub.data$covars)
#' 
#' LUR <- list(covar_fun, covar_fun, covar_fun)
#' 
#' cov.beta <-  list(covf = c("iid", "iid", "iid"), nugget = T)
...
#' locations <- list(coords = c("x","y"), long.lat = c("lon","lat"), others= "type")
#' 
#' sub.model <- createSTmodel(sub.data, LUR = LUR,
#'                             ST = c("bc_st_no2", "bc_st_smk"),
#'                             cov.beta = cov.beta, cov.nu = cov.nu,
#'                             locations = locations)
#' sub.model
...
bc_st_smk <- select(lur_data2, site_id, sample_week, area_smoke_2sd) %>%
  pivot_wider(id_cols = sample_week,
              names_from = site_id, values_from = area_smoke_2sd) %>%
  as.data.frame()
rownames(bc_st_smk) <- bc_st_smk$sample_week
...
#' -----------------------------------------------------------------------------
#' Combine the data sets
#' Scale the SP predictors here!
#' -----------------------------------------------------------------------------

#' Observations
bc_obs_df <- as.data.frame(bc_obs)
bc_obs_df$week <- rownames(bc_obs)
...
#' Trend data for the model based on long-term NO2 and BC at the central sites
#' These data are stored in the Denver_ST_Model_2.2.rdata object
#' Need to filter this to just the two years we are including right now

#load(here::here("Results", "Denver_ST_Model_4.2.rdata"))

#new_trend <- denver.data4.2$trend %>% 
#  filter(date %in% as.Date(rownames(obs)))
...
#' "final" model selected in "16_LUR_BC_ST_NB_v2.Rmd"
#' -----------------------------------------------------------------------------
...
#E.1 <- predict(sub.model, x, LTA = T, transform="unbiased", pred.var=FALSE)
print(E.1)
