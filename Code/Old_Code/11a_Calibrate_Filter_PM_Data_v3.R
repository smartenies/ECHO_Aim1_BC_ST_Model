...
ggplot(cal_data, aes(y = monitor_mean, x = pm_ug_m3)) +
...
njh_lm <- lm(monitor_mean ~ pm_ug_m3,  
...
#' I25-Denver: R2 = 0.41
i25_lm <- lm(monitor_mean ~ pm_ug_m3,  
...
#' GLOBEVILLE: R2 = 0.53
globe_lm <- lm(monitor_mean ~ pm_ug_m3,  
...
i25_dem <- deming(monitor_mean ~ pm_ug_m3,
                  data = filter(cal_data, monitor_id == "080310027"))
print(i25_dem)
...
fit_data <- filter(cal_data, monitor_id == "080310027") %>% 
  mutate(pm_ug_m3_lmcal = int_lm + pm_ug_m3 * beta_pm_lm,
         pm_ug_m3_demcal = int_dem + pm_ug_m3 * beta_pm_dem)

test <- predict(fit_lm, newdata = fit_data)
...
plot(i25_cal_data$pm_ug_m3, i25_cal_data$monitor_mean,
...
#' Fit linear regression model with temp; adjusted R2 = 0.64
i25_lm1 <- lm(monitor_mean ~ pm_ug_m3 + monitor_temp, data = cal_data2)
summary(i25_lm1)
par(mfrow=c(2,2))
plot(i25_lm1)
par(mfrow=c(1,1))

#' Fit linear regression model with temp + temp_sq; adjusted R2 = 0.62
i25_lm1a <- lm(monitor_mean ~ pm_ug_m3 + monitor_temp + monitor_temp_sq, 
               data = cal_data2)
summary(i25_lm1a)
par(mfrow=c(2,2))
plot(i25_lm1a)
...
i25_lm1_summer <- lm(monitor_mean ~ pm_ug_m3 + monitor_temp, 
...
i25_lm1_fall <- lm(monitor_mean ~ pm_ug_m3  + monitor_temp, 
                   data = cal_data2_fall)
...
i25_lm1_winter <- lm(monitor_mean ~ pm_ug_m3 + monitor_temp, 
...
#' adjusted R2 = 0.65
i25_lm2 <- lm(monitor_mean ~ pm_ug_m3 + camp3 + camp4, 
              data = cal_data3)
summary(i25_lm2)
par(mfrow=c(2,2))
plot(i25_lm2)
...
#' Add temperature
#' Adjusted R2 = 0.63
i25_lm2a <- lm(monitor_mean ~ pm_ug_m3 + monitor_temp + camp3 + camp4, 
               data = cal_data3)
summary(i25_lm2a)
par(mfrow=c(2,2))
plot(i25_lm2a)
...
#' Adjusted R2 = 0.60
i25_lm2b <- lm(monitor_mean ~ pm_ug_m3 + monitor_temp + monitor_temp_sq +
                 camp3 + camp4, 
               data = cal_data3)
summary(i25_lm2b)
par(mfrow=c(2,2))
...
summary(i25_lm2) #R2 = 0.65
summary(i25_lm2a) #R2 = 0.63
summary(i25_lm2b) #R2 = 0.60

AIC(i25_lm2) #AIC = 85.6
AIC(i25_lm2a) #AIC = 87.6
AIC(i25_lm2b) #AIC = 89.5

hist(i25_lm2$residuals)
hist(i25_lm2a$residuals)
hist(i25_lm2b$residuals)

mae(i25_lm2$residuals)
rmse(i25_lm2$residuals)

mae(i25_lm2a$residuals)
rmse(i25_lm2a$residuals)

mae(i25_lm2b$residuals)
rmse(i25_lm2b$residuals)

#' -----------------------------------------------------------------------------
#' Based on AIC, adjusted R2 and the residuals, I'm going to go with model 2 for
#' now
#' 
#' Mean bias = 1.69, RMSE = 2.24
#' -----------------------------------------------------------------------------
...
mae(fit_adjlm$residuals)
rmse(fit_adjlm$residuals)

#' Calibrating the filter data
fit_data_final <- cal_data3 

fit_data_final <- fit_data_final %>% 
  #' Calibrated PM (adjusted linear model)
  mutate(pm_ug_m3_adjlm = predict(fit_adjlm, newdata = fit_data_final)) %>% 
  
  #' Calibrate using crude linear model and deming model
  mutate(pm_ug_m3_lm = int_lm + pm_ug_m3 * beta_pm_lm,
         pm_ug_m3_dem = int_dem + pm_ug_m3 * beta_pm_dem)
...
#' MAE and RMSE- Crude linear model
mae(fit_dem$residuals)
rmse(fit_dem$residuals)
...
filter_data2 <- filter_data2 %>% 
  ...
  #' Calibrated PM (adjusted linear model)
  mutate(pm_ug_m3_adjlm = predict(fit_adjlm, newdata = filter_data2)) %>% 
...
filter_data_comp <- filter_data2 %>% 
  select(filter_id, month, season, pm_ug_m3, pm_ug_m3_adjlm, pm_ug_m3_lm, 
...
ggplot(filter_data_comp, aes(x = as.factor(month), y = pm, fill = as.factor(type))) +
  ...
  scale_fill_viridis(name = "Data Type", discrete = T, 
                     labels = c("pm_ug_m3" = "Raw Data",
                                "pm_ug_m3_adjlm" = "Adjusted linear model",
...
#' Examine those really high filter values
high_pm <- filter(filter_data2, pm_ug_m3 > 50)
glimpse(high_pm)
