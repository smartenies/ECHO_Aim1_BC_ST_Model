...
date_list <- seq.Date(as.Date("2009-01-01"), as.Date("2020-12-31"), by = "day")
...
for(i in 1:length(monitors)) {
  ...
  #' Dates for which we want smoke estimates formatted for smoke plumes
  pm_dates <- gsub("-", "", as.character(date_list))
  
  for(j in 1:length(pm_dates)) {
    if(j %% 100 == 0) print(paste("date", j, "of", length(pm_dates)))
    
    smoke_name <- paste0("hms_smoke", pm_dates[j], "_AEA.csv")
    
    #' Get PM for that day
    mon_day <- filter(mon_df, Date_Local == date_list[j])
    mon_mean <- ifelse(nrow(mon_day) > 0, mean(mon_day$Arithmetic_Mean, na.rm = T), NA)
    
    #' If there's no shapefile, make the smoke variable metric NA
    if(!file.exists(here::here("Data/Smoke_Data", smoke_name))) {
      
      #' If there is a PM2.5 value, calculate metrics
      if(!is.na(mon_mean)) {
        ...
        temp <- data.frame(date = pm_dates[j],
                           monitor = monitors[i],
                           daily_mean_pm = mon_mean,
                           plume_within_50km = NA,
                           pm_exceeds_mean_1sd = ifelse(!is.na(mon_mean), mon_mean >= monthly_metric_1sd, NA),
                           pm_exceeds_mean_2sd = ifelse(!is.na(mon_mean), mon_mean >= monthly_metric_2sd, NA))
        smoke_days <- bind_rows(smoke_days, temp)
        rm(temp, monthly_metric_1sd, monthly_metric_2sd, long_term_mean)
      } else {
        temp <- data.frame(date = pm_dates[j],
                           monitor = monitors[i],
                           daily_mean_pm = mon_mean,
      ...
      #' If there is a file but no data, set the smoke variable to 0
      if (nrow(plumes) == 0) {
        ...
        temp <- data.frame(date = pm_dates[j],
                           ...
                           pm_exceeds_mean_2sd = ifelse(!is.na(mon_mean), mon_mean >= monthly_metric_2sd, NA))
        smoke_days <- bind_rows(smoke_days, temp)
        rm(temp, monthly_metric_2sd, monthly_metric_1sd, long_term_mean)
      
      } else {
        #' See if smoke plumes intersect
        plumes_sf <- st_as_sf(plumes, wkt = "WKT", crs = albers)
        plumes_sf <- filter(plumes_sf, !is.na(st_is_valid(plumes_sf)))
        
        monitors_temp <- filter(mon_df, Date_Local == date_list[j]) 
        
        #' If there is no PM data, use monitor locations and set PM to NA
        if(nrow(monitors_temp) == 0) {
          monitors_temp <- filter(monitor_sp_data, monitor_id == monitors[i]) 
          ...
          temp <- data.frame(date = pm_dates[j],
                             ...
                             pm_exceeds_mean_2sd = NA)
          smoke_days <- bind_rows(smoke_days, temp)
          rm(temp, plume_intersect, monitor_buffer, monitor_sf, monitors_temp)
            ...
            smoke_days <- bind_rows(smoke_days, temp)
          rm(temp, pm_exceeds_2sd, pm_exceeds_1sd, monthly_metric_2sd, monthly_metric_1sd,
             long_term_mean, plume_intersect, monitor_buffer, monitor_sf)
        }
        rm(temp, plumes_sf, monitors_temp)
      }
    }
    rm(plumes)
  }
...
mutate_smoke_days <- smoke_days %>% 
  mutate(smoke_day_1sd = ifelse(plume_within_50km == 0 | pm_exceeds_mean_1sd == 0,
                                0, plume_within_50km * pm_exceeds_mean_1sd),
         smoke_day_2sd = ifelse(plume_within_50km == 0 | pm_exceeds_mean_2sd == 0,
                                0, plume_within_50km * pm_exceeds_mean_2sd),
...
#'------------------------------------------------------------------------------
#' Check dates with missing PM2.5 data
#' Set value to 0 if there are no smoke plumes
#'------------------------------------------------------------------------------

orig_smoke_data <- read_csv(here::here("Data", "Monitor_Smoke_Days_AEA_Orig.csv"))
update_smoke_data <- read_csv(here::here("Data", "Monitor_Smoke_Days_AEA.csv"))

table(orig_smoke_data$plume_within_50km, orig_smoke_data$pm_exceeds_mean_1sd)
table(orig_smoke_data$plume_within_50km, orig_smoke_data$pm_exceeds_mean_2sd)
table(orig_smoke_data$smoke_day_2sd)

table(update_smoke_data$plume_within_50km, update_smoke_data$pm_exceeds_mean_1sd)
table(update_smoke_data$plume_within_50km, update_smoke_data$pm_exceeds_mean_2sd)
table(update_smoke_data$smoke_day_2sd)

sum(is.na(orig_smoke_data$smoke_day_2sd))
sum(is.na(update_smoke_data$smoke_day_2sd))

check <- filter(update_smoke_data, is.na(smoke_day_2sd))

monitor_smoke_days <- left_join(update_smoke_data, monitor_sp_data,
                                by = "monitor_id")
