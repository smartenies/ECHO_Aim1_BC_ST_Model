...
#' Just AADT
bc_lm1 <- lm(log(bc_ug_m3) ~ aadt_100, data = bc_lur_data2)
summary(bc_lm1)
par(mfrow=c(2,2)) 
...
bc_lm2 <- lm(log(bc_ug_m3) ~ aadt_100 + tree_cover_500, data = bc_lur_data2)
summary(bc_lm2)
par(mfrow=c(2,2)) 
...
save(log_bc_lasso3, log_bc_lasso_coef3, 
     file = here::here("Results", "BC_LASSO_Model_3C.Rdata"))
