...
locations_all <- read_csv(here::here("Data", data_name)) %>% 
  filter(!is.na(lon)) %>% 
  filter(indoor == 0) %>% 
  filter(campaign %in% paste0("Campaign", c(1, 2, 3, 4)))

locations_sf1 <- dplyr::select(locations_all, filter_id, campaign, 
                               StartDateLocal, EndDateLocal, lon, lat)
...
central_df$lon <- central_sf1$lon
central_df$lat <- central_sf1$lat

central_df$filter_id <- paste(as.character(central_df$filter_id),
                              seq_along(central_df$filter_id),
                              sep = "_")

#' Join distributed site and central site
locations_sf <- bind_rows(locations_sf1, central_df) %>% 
...
locations_sf2 <- locations_sf %>% 
  ...
  select(site_id_lonlat) %>% 
  distinct(site_id_lonlat)
plot(st_geometry(locations_sf2))
...
filter_ids <- locations_sf2$site_id_lonlat
...
for (i in 1:length(filter_ids)) {
  ...
  point <- filter(locations_sf2, site_id_lonlat == filter_ids[i])
  ...
  for(j in 1:length(date_list)) {
    ...
    temp2 <- data.frame(site_id_lonlat = point$site_id_lonlat,
                        week = date_list[j])
...
locations_cov <- cov_temp %>%  
  select(-c(area_bc:idw_bc)) 

covariates_file_name <- paste0("ST_Covariates_Filters_09_19_AEA.csv")
